...
class EcCanvasTaro extends Component {
  componentDidMount() {
  ...
  initByNewWay(callback) {
    ...
    query
      ...
      .exec((res) => {
        ...
        const canvasNode = memory?.node;
        this.canvasNode = canvasNode;
        const canvasDpr = Taro.getSystemInfoSync().pixelRatio;
        const canvasWidth = memory.width;
        const canvasHeight = memory.height;
  ...
  render() {
    ...
    return (
      <Canvas
        type="2d"
        className={`ec-canvas ${canvasId} ${styles.ecCanvas}`}
        canvasId={canvasId}
